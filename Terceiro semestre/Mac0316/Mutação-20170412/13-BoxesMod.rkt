...
(define-type ExprC
  ...
  [parC (a : ExprC) (b : ExprC)]
  [selC (p : ExprC) (n : number)]
  )
...
(define (desugar [as : ExprS]) : ExprC  
  (type-case ExprS as
    ...
    [parS    (a b) (parC (desugar a) (desugar b))]
    [stS     (p) (selC (desugar p) 1)]
    [ndS     (p) (selC (desugar p) 2)]
    ))

(define (hey [n : number]) : ExprC
   (numC n))
...
#|
(define (mkDiv [l : ExprC] [r : ExprC] [c : number]) : ExprC
    (cond
        [(> l r) (mkDiv (- l r) r (+ c 1))]
        [else (numC c)]))
|#

(define (select [p : Value] [n : number]) : Value
    (cond
        [(parV? p)
           (cond
             [(equal? n 1)
             ...
             [else
                (parV-b p)])]
        [else
            (error 'stSel "O argumento não é um par")]))
  ...
  (type-case ExprC a
    ...
    [seqC (b1 b2) (type-case Result (interp b1 env sto)
                    [v*s (v-b1 s-b1) ; resultado e store retornado por b1
                          (interp b2 env s-b1)])]
    [parC (a b) (type-case Result (interp a env sto)
                    [v*s (v-a s-a) ; resultado e store retornado por a
                       (type-case Result (interp b env s-a)
                          [v*s (v-b s-b) ; resultado e store retornado por b
                             (v*s (parV v-a v-b) s-b)])])]
    [selC (p n) (type-case Result (interp p env sto)
                   [v*s (v-p s-p)
                      (v*s (select v-p n) s-p)])]
    ; aplicação de função
    [appC (f a)
    ...
    [(s-exp-list? s)
     (let ([sl (s-exp->list s)])
       (case (s-exp->symbol (first sl))
         ...
         [(par)  (parS (parse (second sl)) (parse (third sl)))]
         [(st)   (stS (parse (second sl)))]
         [(nd)   (ndS (parse (second sl)))]
