...
public class MeuTST<Value extends Comparable<Value>> {
    ...
    private class Pair implements Comparable<Pair> {
        private String key;
        private Value val;
        private Pair(String key, Value val) {
            this.key = key;
            this.val = val;
        }
    ...
    public Iterable<String> keysWithPrefixByValue(String prefix) {
        ...
        Node<Value> x = get(root, prefix, 0);
        if (x == null) return new Stack<String>();
        if (x.val != null) pq.insert(new Pair(prefix, x.val));
    ...
    private void collect(Node<Value> x, StringBuilder prefix, MinPQ<Pair> pq) {
        if (x == null) return;
        collect(x.left,  prefix, pq);
        if (x.val != null) pq.insert(new Pair(prefix.toString() + x.c, x.val));
