...
public class MeuLinearProbingHashST<Key, Value> {
    ...
    */
    ...
    public int maxCluster() {
        ...
        if (vals[m-1] != null) {
            int i = 0;
            while (vals[i] != null && i < m)
                i++;
            if (count+i > maxClustSize)
                maxClustSize = count+i;
        }
        return maxClustSize;
    }
    ...
    public int numClusters() {
        ...
        boolean gotIt = false;
        for (int i = 0; i < m; i++) {
            if (vals[i] != null && !gotIt) {
                gotIt = true;
            }
            else if (vals[i] == null && gotIt) {
                gotIt = false;
                clusters++;
            }
        }
    ...
    public double averageSearchHit() {
        ...
        for (Key key : keys()) {
            count = 1;
            for (int i = hash(key); !keys[i].equals(key); i = (i + 1) % m)
                count++;
            sum += count;
        }
    ...
    ***********************************************************************/
    public static void main(String[] args) {
        ...
        while (!in.isEmpty()) {
            ...
            String[] chaves = linha.split("\\W+");
            for (int i = 0; i < chaves.length; i++) {
                //StdOut.print("Key: " + chaves[i]);
                //StdOut.println("Alfa: " + meuST.size()/(double)meuST.sizeST());
